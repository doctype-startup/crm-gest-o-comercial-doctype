...
const dateLabel = (value: unknown) => { const iso = text(value); return /^\d{4}-\d{2}-\d{2}$/.test(iso) ? new Date(`${iso}T12:00:00`).toLocaleDateString("pt-BR") : "—"; };
  ...
  return <>
    ...
    {view && <div className="commercial-page commercial-suite" data-view={view}>
      ...
      <div className="commercial-table"><table><thead><tr>{view === "products" ? <><th>Produto</th><th>SKU</th><th>Categoria</th><th>Preço</th><th>Custo</th><th>Margem</th><th>Cobrança</th><th>Status</th></> : view === "quotes" ? <><th>Nº</th><th>Cliente</th><th>Produtos</th><th>Total</th><th>Validade</th><th>Status</th></> : <><th>Nº</th><th>Cliente</th><th>Produtos</th><th>Valor</th><th>Vigência</th><th>Documento</th><th>Status</th></>}<th>Ações</th></tr></thead><tbody>
        {filtered.map((record) => <tr key={record.id}>{view === "products" ? <><td><strong>{text(record.data.name)}</strong><small>{text(record.data.description)}</small></td><td>{text(record.data.sku) || "—"}</td><td>{text(record.data.category) || "—"}</td><td>{money(record.data.price)}</td><td>{money(record.data.cost)}</td><td>{Number(record.data.price || 0) ? `${Math.round(((Number(record.data.price || 0) - Number(record.data.cost || 0)) / Number(record.data.price || 1)) * 100)}%` : "—"}</td><td>{text(record.data.billingType)}</td><td><Status value={text(record.data.status)} /></td></> : view === "quotes" ? <><td>{text(record.data.number)}</td><td>{clientName(record.data.clientId)}</td><td>{productNames(record.data.productIds)}</td><td>{money(record.data.total)}</td><td>{dateLabel(record.data.validUntil)}</td><td><Status value={text(record.data.status)} /></td></> : <><td>{text(record.data.number)}</td><td>{clientName(record.data.clientId)}</td><td>{productNames(record.data.productIds)}</td><td>{money(record.data.value)}</td><td>{dateLabel(record.data.startDate)}{record.data.endDate ? ` → ${dateLabel(record.data.endDate)}` : ""}</td><td>{record.data.fileDataUrl ? <a className="contract-download" href={text(record.data.fileDataUrl)} download={text(record.data.fileName) || "contrato.pdf"}><Download size={14} /> {text(record.data.fileName) || "Baixar"}</a> : "—"}</td><td><Status value={text(record.data.status)} /></td></>}<td><div className="commercial-row-actions"><button aria-label="Editar" onClick={() => setModal({ module: view, record })}><Pencil size={15} /></button><button aria-label="Excluir" className="danger" onClick={() => remove(record)}><Trash2 size={15} /></button></div></td></tr>)}
...
function Field({ label, value, set, type = "text", required, disabled }: { label: string; value: unknown; set: (v: string) => void; type?: string; required?: boolean; disabled?: boolean }) {
  if (type === "date") return <DateField label={label} value={text(value)} onChange={set} required={required} />;
  return <label><span>{label}</span><input type={type} step={type === "number" ? "0.01" : undefined} value={text(value)} required={required} disabled={disabled} onChange={(e) => set(e.target.value)} /></label>;
}
