...
export function SubscriptionView() {
  ...
  return <div className="subscription-page stack">
    ...
    <section className="subscription-card"><header><div><CheckCircle2 /><span><small>Status da cobrança</small><strong className={paymentClass}>{subscription.paymentStatus}</strong></span></div><p>Renovação contratual: <b>{dateLabel(subscription.renewalDate)}</b></p></header><div className="subscription-details"><p><Mail /><span><b>E-mail financeiro</b>{subscription.billingEmail}</span></p><p><CalendarDays /><span><b>Prazo de regularização</b>{dateLabel(subscription.graceUntil)}</span></p><p><ShieldCheck /><span><b>Privacidade</b>Outras empresas não conseguem visualizar esta assinatura.</span></p></div><div className="billing-activation"><div className="billing-activation-icon"><QrCode /></div><div><span>{subscription.testMode ? "AMBIENTE DE TESTE" : "PAGAMENTO SEGURO"}</span><h3>Cobrança automática</h3><p>Autorize uma vez com Pix ou Cartão de crédito na página segura da Stripe. As próximas mensalidades são cobradas automaticamente no mesmo método.</p></div>{subscription.automaticBilling ? <div className="billing-activation-active"><CheckCircle2 /> Ativo via {subscription.paymentMethod}</div> : <button className="primary" disabled={activating || !subscription.monthlyPrice || !subscription.stripeConfigured} onClick={() => void activateBilling()}>{activating ? "Abrindo Stripe…" : "Ativar cobrança automática"}</button>}</div>{error && <div className="subscription-error">{error}</div>}<footer>A DOCTYPE não recebe nem armazena senha bancária, chave Pix, número de cartão ou dados da conta. A autorização e as cobranças são processadas pela Stripe.</footer></section>
  </div>;
}
