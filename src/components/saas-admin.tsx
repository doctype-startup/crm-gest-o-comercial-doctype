...
const blank: Form = { name: "", slug: "", logoDataUrl: "", plan: "Start", status: "Teste", maxUsers: 3, renewalDate: "", notes: "", adminName: "", adminEmail: "", temporaryPassword: "" };
...
export function SaasAdmin({ notify }: { notify: (message: string) => void }) {
  ...
  return <div className="saas-admin stack">
    ...
    <div className="saas-kpis"><article><span>Empresas ativas</span><strong>{active}</strong><small>{organizations.length} contas cadastradas</small></article><article><span>Em período de teste</span><strong>{trials}</strong><small>Conversões em acompanhamento</small></article><article><span>Usuários ativos</span><strong>{activeUsers}</strong><small>{usedSeats} usuários cadastrados</small></article><article><span>Licenças utilizadas</span><strong>{usedSeats}/{totalSeats}</strong><small>Capacidade contratada</small></article></div>
...
function OrganizationCard({ organization, edit }: { organization: SaasOrganization; edit: () => void }) {
  ...
  return <article className="saas-card">
    ...
    <div className="saas-plan"><span>Plano<strong>{organization.plan}</strong></span><span>Renovação<strong>{dateLabel(organization.renewalDate)}</strong></span></div>
...
function OrganizationModal({ value, close, saved }: { value: SaasOrganization | "new"; close: () => void; saved: () => Promise<void> }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      const payload = current ? { name: form.name, slug: form.slug, logoDataUrl: form.logoDataUrl, plan: form.plan, status: form.status, maxUsers: form.maxUsers, renewalDate: form.renewalDate, notes: form.notes } : form;
