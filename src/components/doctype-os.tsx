...
type ManagedUser = { id: string; name: string; email: string; role: Role; active: boolean; mustChangePassword: boolean; permissions: { read: RecordModuleKey[]; write: RecordModuleKey[] }; lastLoginAt: string | null; sessionActive: boolean };
...
const dateTimeLabel = (value: unknown) => value ? new Date(String(value)).toLocaleString("pt-BR", { dateStyle: "short", timeStyle: "short" }) : "—";
function relativeTime(value: unknown) {
  if (!value) return "Nunca acessou";
  const diffMs = Date.now() - new Date(String(value)).getTime();
  ...
  return dateLabel(String(value).slice(0, 10));
}
...
async function api<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    const fieldMessages: string[] = body.fields ? Object.values(body.fields).flat().filter((message): message is string => Boolean(message)) : [];
    throw new Error(fieldMessages.length ? fieldMessages.join(" ") : body.error || "Não foi possível concluir a operação.");
  }
  return body;
}
...
function SettingsView({ goal, taxRate, onSaved, downloadBackup, importRef, restoreBackup, user, notify }: { goal: number; taxRate: number; onSaved: () => void; downloadBackup: () => Promise<void>; importRef: React.RefObject<HTMLInputElement | null>; restoreBackup: (file: File) => Promise<void>; user: SessionUser; notify: (m: string) => void }) {
  const [crmGoal, setCrmGoal] = useState(goal); const [taxRateInput, setTaxRateInput] = useState(taxRate); const [users, setUsers] = useState<ManagedUser[]>([]); const [usersError, setUsersError] = useState(""); const [userModal, setUserModal] = useState<ManagedUser | "new" | null>(null); const [passwordModal, setPasswordModal] = useState(false); const [sendingTest, setSendingTest] = useState(false);
