...
export function DoctypeOS({ initialState }: { initialState: StatePayload }) {
  ...
  async function saveRecord(module: ModuleKey, data: Record<string, unknown>, id?: string) {
    await api(id ? `/api/records/${id}` : "/api/records", { method: id ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ module, data }) });
