...
function SettingsView({ goal, taxRate, onSaved, downloadBackup, importRef, restoreBackup, user, notify }: { goal: number; taxRate: number; onSaved: () => void; downloadBackup: () => Promise<void>; importRef: React.RefObject<HTMLInputElement | null>; restoreBackup: (file: File) => Promise<void>; user: SessionUser; notify: (m: string) => void }) {
  ...
  async function sendTestNotification() { setSendingTest(true); setUsersError(""); try { await api("/api/notifications/test", { method: "POST" }); notify("Notificação de teste enviada — confira o sino e seu e-mail."); } catch (e) { setUsersError(e instanceof Error ? e.message : "Não foi possível enviar a notificação de teste."); } finally { setSendingTest(false); } }
