...
import type { RecordModuleKey, SessionUser } from "./types";

export async function getAppState(user: SessionUser) {
  const modules: RecordModuleKey[] = ["clients", "accesses", "invoices", "expenses", "tasks", "crm", "team", "products", "quotes", "contracts"];
