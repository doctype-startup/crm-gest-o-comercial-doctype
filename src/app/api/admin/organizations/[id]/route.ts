...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await db.transaction().execute(async (trx) => {
      ...
      await trx.updateTable("saas_accounts").set({ slug: body.slug, logo_data_url: body.logoDataUrl, plan: body.plan, status: body.status, max_users: body.maxUsers, renewal_date: body.renewalDate, notes: body.notes, is_test_client: body.isTestClient ? 1 : 0, updated_at: now }).where("org_id", "=", id).execute();
