...
import { saasAccountSchema } from "@/lib/saas";
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const body = saasAccountSchema.parse(await request.json());
    ...
    await audit(session.orgId, session.id, "UPDATE", "saas_organization", id, { name: body.name, plan: body.plan, previousStatus: current.status, status: body.status });
