...
export async function GET() {
  try {
    ...
    const rows = await db
      ...
      .select(["o.id", "o.name", "sa.slug", "sa.logo_data_url", "sa.plan", "sa.status", "sa.max_users", "sa.renewal_date", "sa.notes", "sa.is_test_client", "sa.created_at", "sa.updated_at", "sb.monthly_price", "sb.billing_cycle", "sb.billing_day", "sb.billing_email", "sb.payment_method", "sb.payment_status", "sb.next_charge_date", "sb.grace_until"])
      .orderBy("o.name")
      .execute();
...
export async function POST(request: Request) {
  try {
    ...
    await db.transaction().execute(async (trx) => {
      ...
      await trx.insertInto("saas_accounts").values({ org_id: orgId, slug: body.slug, logo_data_url: body.logoDataUrl, plan: body.plan, status: body.status, max_users: body.maxUsers, renewal_date: body.renewalDate, notes: body.notes, is_test_client: body.isTestClient ? 1 : 0, created_at: now, updated_at: now }).execute();
