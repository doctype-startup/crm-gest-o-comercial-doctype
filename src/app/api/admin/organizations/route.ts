...
export async function GET() {
  try {
    ...
    const rows = await db
      ...
      .select(["o.id", "o.name", "sa.slug", "sa.logo_data_url", "sa.plan", "sa.status", "sa.max_users", "sa.renewal_date", "sa.notes", "sa.created_at", "sa.updated_at"])
      .orderBy("o.name")
      .execute();
...
export async function POST(request: Request) {
  try {
    ...
    await audit(session.orgId, session.id, "CREATE", "saas_organization", orgId, { name: body.name, slug: body.slug, plan: body.plan, status: body.status, adminEmail: body.adminEmail.toLowerCase() });
