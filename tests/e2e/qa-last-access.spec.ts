...
test("mostra último acesso e sessão ativa na tabela de usuários", async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole("columnheader", { name: "Último acesso" })).toBeVisible();
  await expect(page.getByText("Sessão ativa")).toBeVisible();
  await page.screenshot({ path: `test-results/qa-last-access-${testInfo.project.name}.png`, fullPage: true });
});
